...
#[derive(Parser)]
#[command(name = "vela", version = "0.12.0")]
...
pub async fn run_command() {
    ...
    match Cli::parse().command {
        ...
        Commands::Version => println!("vela 0.12.0"),
...
pub async fn cmd_compile(
    ...
    if local_source.exists() {
        match corpus::compile_local_corpus(local_source, output, backend).await {
            Ok(report) => {
                println!();
                println!("  {}", "VELA · COMPILE · V0.12.0".dimmed());
...
fn cmd_stats(path: &Path) {
    ...
    println!();
    println!("  {}", "FRONTIER · V0.12.0".dimmed());
...
async fn cmd_bridge(inputs: &[PathBuf], check_novelty: bool, top_n: usize) {
    ...
    println!();
    println!("  {}", "VELA · BRIDGE · V0.12.0".dimmed());
    println!("  {}", style::tick_row(60));
...
async fn cmd_jats(source: &str, output: &Path, backend: Option<&str>) {
    ...
    let client = Client::new();
    println!();
    println!("  {}", "VELA · JATS · V0.12.0".dimmed());
    println!("  {}", style::tick_row(60));
