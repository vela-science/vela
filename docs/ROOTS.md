...
| Producer identity-binding handle `vib_…` | First 16 hex characters of SHA-256 over the Vela-canonical `vela.identity_binding.v0.1` object with `binding_id` and `signature` cleared. The self-signature separately proves possession of the embedded Ed25519 key. | Exact, unique handle for routing; use the complete preimage root if a future accepted contract authorizes the credential. | Actor-registry membership, full credential identity, personhood, expertise, independence, or authority by itself. |
