...
test("current product release pins the tested Vela and Codex boundaries", async () => {
  ...
  assert.equal(SUPPORTED_VELA_VERSION, "0.940.4");
  assert.equal(SUPPORTED_CODEX_VERSION, "0.145.0");
  assert.match(workflow, /releases\/download\/v0\.940\.2/u);
  assert.match(workflow, /CANOPUS_VELA_VERSION="0\.940\.2"/u);
...
test("current source stays product-only while historical release evidence remains linked", async () => {
  ...
  assert.equal(packageJson.version, "0.8.0-rc.3");
  for (const file of [
    "README.md",
    "THIRD_PARTY.md",
    "docs/MISSIONS.md",
  ...
  assert.match(
    readme,
    /Current source is Canopus `0\.8\.0-rc\.3`, composed with released Vela[\s\S]+`0\.940\.4`/u,
  );
