...
test("registered product profiles stage exact distinct capsules and bounded Mission v1 drafts", async () => {
  ...
  const profiles = [
    await loadProductProfile("erdos1056-k15-10428801-10429000", { platform: "darwin-arm64" }),
    await loadProductProfile("erdos1056-k15-10428801-10429000", { platform: "linux-x86_64" }),
...
test("profile v2 binds exact platform custody and packs only portable contract resources", async () => {
  ...
  const name = "erdos1056-k15-10428801-10429000";
...
test("explicit targets are deliberate while the default never skips rank one", async () => {
  const profile = await loadProductProfile("erdos1056-k15-10428801-10429000");
  const offer = {
    targets: [
...
test("ordinary profile discovery selects the unique first-offer profile", async () => {
  ...
  assert.equal(profile.name, "erdos1056-k15-10428801-10429000");
