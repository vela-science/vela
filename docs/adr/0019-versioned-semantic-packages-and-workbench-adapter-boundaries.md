...
- 2026-08-03 package-ecosystem refinement: preserve four separate planes:
  activity, scientific state, reusable packages, and discovery. The first
  candidate is `vela-science/lean-replay-contract@0.0.0-source-local`, but its
  exact identity is its JCS/SHA-256 package root, not the coordinate, a URL, a
  Git branch, or a database row. Do not introduce `vela://`, `pkg:vela`, a
  `vela.lock` in current Frontiers, publisher namespaces, OCI distribution, a
  Web index, or a hosted registry. Native package managers and workbenches keep
  their own locks and execution semantics. Package impact may later orient
  Frontier review, but only each Frontier's attributed human Decision may
  change Standing.
...
Operationally, the package layer remains distinct from activity and discovery:

```text
activity (runs, worktrees, logs, compute)
  -> package roots (reusable contracts; authority_effect none)
  -> scientific state (Submission, Verification, human Decision, Standing)
  -> discovery (Observatory, Atlas, search, package pages)
```
...
under [`research/lean-replay-contract/`](../../research/lean-replay-contract/)
in this repository as a source-local experiment. Extraction to a shared
repository is permitted only when:
...
There is no hosted package registry in the first program. Before the Level 1
gate, consumers retain an exact source commit, tree, path, logical package root,
and rooted file descriptor. After the gate, immutable Git releases are
sufficient. Native toolchain locks remain owned by their native ecosystems; no
Vela package lock enters canonical Frontier state.
