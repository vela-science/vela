...
EXTERNAL_LEAN_REPLAY_SCHEMA = "vela.external_lean_replay.v1"
EXTERNAL_LEAN_ENVIRONMENT_DIR = "external-lean"
...
def source_snapshot_path(relative: Any) -> str:
    ...
    Retained external Lean bundles may include three declarative Lake metadata
    files beside their Lean modules. Only `.lean` paths are elaborated; these
    other exact filenames remain hash-bound provenance.
    """
...
def measure_external_lean_environment(
    packet: dict[str, Any],
...
def approved_external_lean_environment(
    packet: dict[str, Any],
    environment_root: Path,
    ...
    actual, runtime_paths = measure_external_lean_environment(packet, environment_root)
    if contract != actual:
        raise ValueError(
...
def environment_contract_candidate(
    ...
    if packet.get("schema") != EXTERNAL_LEAN_REPLAY_SCHEMA or any(
    ...
    candidate, _runtime_paths = measure_external_lean_environment(
        packet,
        ROOT / "envs" / EXTERNAL_LEAN_ENVIRONMENT_DIR / workspace_key,
    )
    return candidate
...
def verify(packet_path: Path, *, bundle_root: Path | None = None) -> dict[str, Any]:
    ...
    if packet.get("schema") != EXTERNAL_LEAN_REPLAY_SCHEMA:
    ...
    environment_root = ROOT / "envs" / EXTERNAL_LEAN_ENVIRONMENT_DIR / workspace_key
    ...
    environment_contract, dependency_lean_paths = approved_external_lean_environment(
        packet,
        environment_root,
        lineage,
    )
    ...
    write_text(workspace / "_VelaExternalLeanAudit.lean", audit_source)
    lean, audit_sandbox = run_sandboxed_lean_command(
        ["lean", "-R", str(workspace), "_VelaExternalLeanAudit.lean"],
        workspace,
        timeout=600,
        source_root=source_root,
        toolchain=toolchain,
