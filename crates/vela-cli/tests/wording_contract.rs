...
/// The trees where the retired noun is the record rather than the drift.
///
/// A memo is what was thought on the day it was written, a superseded ADR is
/// what was decided before it was superseded, and a release note is a dated
/// statement about a release that shipped. Editing any of them to match today's
/// vocabulary would be falsifying the record, and AGENTS.md separately forbids
/// touching release notes in an ordinary change.
const NOT_CURRENT_PRODUCT_PROSE: [&str; 5] = [
    "docs/history/",
    "docs/adr/",
    "paper/",
    "research/",
...
/// The current documents that may still say it, and why each one may.
///
/// Deliberately a list of files rather than a list of lines. A line allowlist
/// would freeze whatever is wrong today — which is the failure the CLI half of
/// this file exists to avoid, and the reason this sweep was the second one.
/// A document not named here may not say the word at all, so a new document is
/// covered by writing it rather than by remembering to extend a list.
///
/// The reason strings are the point. Adding an entry costs a sentence
/// defending it, which is the friction that keeps the list short.
const WHY_THE_WORD_STAYS: [(&str, &str); 13] = [
    (
        "README.md",
        "states the derived-query definition and links ADR 0028 by title",
    ),
    ("docs/README.md", "links ADR 0028 by title"),
    (
        "docs/TERMINOLOGY.md",
        "defines the term and carries the retired-names table",
    ),
    (
        "docs/ECOSYSTEM.md",
        "defines the term, records the vfr_/frontier_id retirement, and names the archived epoch-1 repositories",
    ),
    (
        "docs/ARCHITECTURE.md",
        "states the definition and names the archived epoch-1 repositories",
    ),
    (
        "docs/CONTINUITY.md",
        "names the archived epoch-1 repositories and their frontier.toml",
    ),
    (
        "docs/REPOSITORY_PROFILE.md",
        "names the archived repositories, their retired files, and the action's retired `frontier` input",
    ),
    (
        "docs/CLI.md",
        "quotes the repository name quantum-codes-frontier in sample output",
    ),
    (
        "docs/PROTOCOL.md",
        "groups product navigation over derived state, and names the `frontier` key the schema rejects",
    ),
    (
        "docs/INTEROPERABILITY.md",
        "names the `frontier` key the schema rejects",
    ),
    (
    ...
    (
        "conformance/fixtures/epoch1/README.md",
        "describes epoch-1 fixture bytes, which carry frontier_id",
    ),
    (
        "crates/vela-verify/corpus/valid/README.md",
        "names the repositories the corpus came from",
    ),
];

/// Tracked Markdown, as repository-relative slash-separated paths.
///
/// `git ls-files` rather than a directory walk: the walk would have to learn
/// every ignored tree that grows one — `target/`, a `.venv/` a test run leaves
/// behind — and a guard that reports a build artifact is a guard people switch
/// off. Tracked files are also exactly the ones a reader receives.
fn tracked_markdown(root: &Path) -> Vec<String> {
    ...
    let paths: Vec<String> = String::from_utf8(listed.stdout)
        .expect("paths must be UTF-8")
        .split('\0')
    ...
    assert!(
        paths.len() > 40,
        "only {} tracked Markdown files found; this test would cover almost nothing",
        paths.len()
    );
    paths
}

/// No current document may call a Frontier the thing that holds the bytes.
///
/// ADR 0039 made the Repository the authority boundary and left the Frontier a
/// derived query with no identifier. `no_help_body_names_a_frontier` above holds
/// the binary to that; nothing held the prose, so the 0.971.0 sweep fixed twelve
/// sentences and the definitional claims in THEORY, SIGNING, PUBLISHING,
/// THREAT_MODEL, VERIFICATION and AGENTS.md outlived it by two releases.
///
/// The word is banned as a substring, not as a whole word, for the reason the
/// CLI half gives: the drift arrived as `frontier-relative`, `frontier-local`
/// and `four Frontiers`, not as the bare noun. Banning definitional *phrasings*
/// instead would be a list of the sentences that were wrong once, and the next
/// one will be phrased differently.
///
/// Markdown only. The remaining hits under `crates/`, `scripts/` and
/// `conformance/` are overwhelmingly retired-spelling detectors (this file
/// included), epoch-1 fixture keys, archived repository names, and one
/// consumer's own free-form `kind` value; `scripts/ecosystem-status.py`'s
/// `RETIRED_SPELLINGS` already guards the half of that with a machine consumer.
/// Widening this to `.rs` would drown the signal.
#[test]
fn no_current_document_names_a_frontier() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../");
    let allowed: BTreeSet<&str> = WHY_THE_WORD_STAYS.iter().map(|(path, _)| *path).collect();

    let mut offenders = Vec::new();
    let mut allowlist_earned: BTreeSet<&str> = BTreeSet::new();

    for path in tracked_markdown(&root) {
        if NOT_CURRENT_PRODUCT_PROSE
            .iter()
        ...
        match allowed.get(path.as_str()) {
            Some(entry) => {
                allowlist_earned.insert(entry);
            }
            None => offenders.extend(hits),
        }
    }

    assert!(
        offenders.is_empty(),
        "these documents say Frontier where ADR 0039 means Repository. Fix the \
         prose, or add the file to WHY_THE_WORD_STAYS with the reason it may:\n{}",
        offenders.join("\n")
    );

    /* An allowlist entry that stops being needed is an exemption nobody is
    checking, and the next document to earn that name inherits it silently. */
    let unearned: Vec<&str> = WHY_THE_WORD_STAYS
        .iter()
        .map(|(path, _)| *path)
        .filter(|path| !allowlist_earned.contains(path))
        .collect();
    assert!(
        unearned.is_empty(),
        "WHY_THE_WORD_STAYS exempts documents that no longer say it; delete these \
         entries:\n  {}",
        unearned.join("\n  ")
    );
}
