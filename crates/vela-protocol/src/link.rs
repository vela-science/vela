...
/// Maximum number of findings to send to the LLM in a single link-inference batch.
...
pub async fn infer_links(
    ...
    for start in (0..bundles.len()).step_by(LLM_LINK_BATCH_SIZE) {
        ...
        // Try up to 2 times
        for attempt in 0..2 {
            match llm::call(client, config, LINK_PROMPT, &summary).await {
                Ok(raw) => match llm::parse_json(&raw) {
                    Ok(parsed) => {
                        ...
                        for rl in &links {
                                ...
                                && VALID_LINK_TYPES.contains(&lt)
                            {
                                ...
                                if existing_links.contains(&(from_id.clone(), target_id.clone())) {
                                    // Find and merge with existing link.
