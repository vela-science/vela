...
```sh
curl -fsSL https://raw.githubusercontent.com/vela-science/vela/v0.915.0/install.sh \
  | VELA_VERSION=v0.915.0 bash
vela --version
```
...
```powershell
& ([scriptblock]::Create((Invoke-WebRequest https://raw.githubusercontent.com/vela-science/vela/v0.915.0/install.ps1).Content)) -Version v0.915.0
vela --version
```
...
`cargo install --locked vela-cli --version 0.915.0`. Human signing setup is
