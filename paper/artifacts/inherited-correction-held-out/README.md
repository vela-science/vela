...
This non-authoritative paper artifact is a prospective follow-up to the sealed
16-session study. That study and its `positive_gate=not_supported` result are
unchanged. No new participant call has been made.
...
```bash
python3 paper/artifacts/inherited-correction-held-out/benchmark.py verify
python3 paper/artifacts/inherited-correction-held-out/custody.py verify-prelaunch
python3 paper/artifacts/inherited-correction-held-out/test_benchmark.py
git diff --check
```
