...
ORCHESTRATOR = Path(
    "/Users/williamblair/Documents/Codex/2026-08-21/realtime-voice-chat-2/work/stage_a_anthropic_once/orchestrator"
)
...
def main() -> int:
    ...
    schema = (
        PACKAGE / "offline-qualification-assets/anthropic-provider_schema.json"
    ).read_bytes()
    ...
    openai = json.loads(
        (PACKAGE / "offline-qualification-assets/openai-held_permit.json").read_bytes()
    )
    ...
    if prelaunch.get("fixed_denominator") != 12 or any(
        prelaunch[key] != 0
        for key in (
            "provider_calls",
            "released_permits",
    ...
    if (
        held["run_id"] != RUN_ID
        or held["status"] != "held"
        or held["consumed_at"] is not None
    ):
    ...
    try:
        ...
        if consumed.name != f"{RUN_ID}.permit.consumed.json" or sha256(
            consumed.read_bytes()
        ) != sha256(canonical_pretty(held)):
