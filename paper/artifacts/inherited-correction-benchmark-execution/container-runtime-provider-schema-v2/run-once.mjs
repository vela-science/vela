...
function deriveProviderSchema(schema) {
  const provider = structuredClone(schema);
  const bindings = provider?.properties?.evidence_bindings;
  if (!bindings || bindings.uniqueItems !== true) throw new Error("registered_unique_items_missing");
  delete bindings.uniqueItems;
  return provider;
}
...
function verifyBindings(runId, permit, config, assignment, promptBytes, schemaBytes, providerSchemaBytes, trustBundleBytes) {
  ...
  if (config.provider_response_schema_bytes !== sha(providerSchemaBytes)) throw new Error("provider_schema_bytes");
  const registeredSchema = JSON.parse(schemaBytes.toString("utf8"));
  const providerSchema = JSON.parse(providerSchemaBytes.toString("utf8"));
  if (canonical(providerSchema) !== canonical(deriveProviderSchema(registeredSchema))) throw new Error("provider_schema_derivation");
...
async function main() {
  ...
  const providerSchemaBytes = bytes("/input/provider-response-schema.json");
  const trustBundleBytes = bytes(TRUST_BUNDLE_PATH);
  const exact = verifyBindings(runId, permit, config, assignment, promptBytes, schemaBytes, providerSchemaBytes, trustBundleBytes);
  ...
  const cliArgs = [
    ...
    ...configArgs(),
    "--sandbox", "read-only", "--cd", "/work", "--output-schema", "/input/provider-response-schema.json",
  ...
  const receipt = {
    ...
    registered_response_schema_bytes: sha(schemaBytes), provider_response_schema_bytes: sha(providerSchemaBytes),
...
export { compileResponseSchema, deriveProviderSchema, forbiddenEvent, inspectEvents, root, sha, validateResponseAgainstSchema, verifyBindings };
