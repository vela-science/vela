...
//! read-only math-atlas anchor listing `vela state anchors`.
//!
...
//! sit ahead of the clap dispatcher. The parser consumes the public `state`
//! argv shape directly; no retired `claim` spelling or compatibility rewrite
//! survives.
...
/// public argv vector: `vela state <frontier> <vf_…>` for the state cell, or
/// `vela state trust|pack|diff <frontier> <id>` for a named projection.
pub(crate) fn run(args: &[String]) {
    let (verb, operand_start) = match args.get(2).map(String::as_str) {
        Some("trust" | "pack" | "diff") => (args[2].as_str(), 3),
        _ => ("state", 2),
    };
    let json = args.iter().any(|a| a == "--json");
    // Positional operands: the first two non-flag tokens after the optional
    // named projection.
    ...
    match verb {
        ...
        other => fail_usage(&format!("unknown state projection '{other}'")),
    }
}

/// `vela state anchors <frontier> [vf_id]` — list accepted math-atlas anchor
/// links. Anchor writes must enter through Receipt v1 and the shared policy
/// edge; the state namespace is projection-only.
pub(crate) fn run_anchors(args: &[String]) {
    use vela_protocol::anchor::AnchorLink;
    ...
    let frontier = positionals
        .first()
        .copied()
        .unwrap_or_else(|| fail_usage("usage: vela state anchors <frontier> [vf_id] [--json]"));
    let project = repo::load_from_path(Path::new(frontier)).unwrap_or_else(|e| fail(&e));
    let target = positionals.get(1).copied();
    let links: Vec<&AnchorLink> = project
        .anchor_links
        .iter()
        .filter(|link| target.is_none_or(|id| link.target == id))
        .collect();
    if json {
        ...
        println!(
            "no anchor links{}",
            target.map(|id| format!(" on {id}")).unwrap_or_default()
        );
    } else {
        for link in links {
            println!(
                ...
                link.id,
                link.target,
                link.anchor.namespace,
                link.anchor.id,
                link.anchor.role,
                link.anchor.join_policy
            );
        }
    }
}
