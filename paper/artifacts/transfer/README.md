...
The frozen source audit in `plan.json` and `result.json` established that
...
`erdos-424/receiver-preflight.json`, byte root
...
`erdos-424/receiver-publication.json`, byte root
...
1. `reference.json` is the minimal rooted native manifest. It retains the
...
`vela-loss-report.json` states the six Vela semantic planes that base
RO-Crate does not preserve losslessly. `reader-result.json` records the
existing independent native verifier and a separate standard-library
RO-Crate reader. `result.json` records two byte-identical clean rebuilds and
...
```text
plan root
sha256:b719174a729fa49489705e70dc882c7be44486b61082abaaaaf9b9f77d40347f
...
uv run --project conformance --locked python \
  ...
  --plan paper/artifacts/transfer/scientific-change-package-plan.json \
  --plan-amendment \
    paper/artifacts/transfer/scientific-change-package-plan-amendment-001.json \
  --evidence-amendment \
    paper/artifacts/transfer/scientific-change-package-plan-amendment-002.json
