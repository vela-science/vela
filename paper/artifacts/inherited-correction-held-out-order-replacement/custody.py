...
def static_state() -> dict[str, Any]:
    ...
    for family_id, family in benchmark.family_map().items():
        for condition in benchmark.CONDITIONS:
            ...
            if (
                benchmark.byte_digest(
                    (input_dir / "provider-response-schema.json").read_bytes()
                )
                != bindings["provider_response_schema_bytes"]
            ):
                raise CustodyError("provider_schema_copy_drift")
    return {
        "benchmark": benchmark,
...
def verify_prelaunch() -> dict[str, Any]:
    ...
    if preregistration["status"] != benchmark.CURRENT_STATUS:
    ...
    if (
        amendment.get("status") != "authorized_held_pending_schema_compatibility_review"
    ...
    if (
        order_amendment.get("status")
        != "authorized_held_pending_independent_order_review"
        or order_amendment.get("stopped_registration", {}).get(
            "stop_evidence_commit"
        )
        != "f14616e341929e7ad74927a846cba12e5889154e"
        or order_amendment.get("repair", {}).get(
            "response_order_semantically_relevant"
        )
        is not False
        or order_amendment.get("successor_execution_state")
        != {
            "sessions_completed": 0,
            "fixed_denominator": 36,
        ...
        or order_amendment.get("neutral_calibration", {}).get("required") is not True
    ):
        raise CustodyError("order_contract_amendment_invalid")
    ...
    permits = sorted(
        (ROOT / "permit-template").glob(f"{benchmark.RUN_PREFIX}-*.permit.json")
    )
    if len(permits) != 36:
    ...
    calibration = freeze.get("neutral_calibration")
    calibration_config = load_json(
        ROOT / "calibration/input/participant-configuration.json"
    )
    calibration_assignment = load_json(ROOT / "calibration/input/assignment.json")
    calibration_permit = load_json(
        ROOT
        / f"calibration/permit-template/{benchmark.CALIBRATION_RUN_ID}.permit.json"
    )
    calibration_hold = load_json(ROOT / "calibration/permit-template/hold-state.json")
    if (
        not isinstance(calibration, dict)
        or calibration.get("status") != "held"
        or calibration.get("provider_calls") != 0
        or calibration.get("denominator_credit") is not False
        or calibration.get("configuration_root")
        != benchmark.canonical_root(calibration_config)
        or calibration.get("assignment_root")
        != benchmark.canonical_root(calibration_assignment)
        or calibration.get("permit_root")
        != benchmark.canonical_root(calibration_permit)
        or calibration_permit.get("status") != "held"
        or calibration_permit.get("expires_at") != "not_authorized"
        or calibration_hold.get("status") != "hold"
        or list((ROOT / "calibration/permit-template").glob("*.consumed.json"))
    ):
        raise CustodyError("neutral_calibration_hold_invalid")
    return {
...
def validate_capture(capture_dir: Path, run_id: str) -> dict[str, Any]:
    ...
    expected_receipt = {
        ...
        "trust_bundle_bytes": runtime["trust_bundle_bytes"],
        "registered_response_schema_bytes": preregistration["bindings"][
            "response_schema_bytes"
        ],
        "provider_response_schema_bytes": preregistration["bindings"][
            "provider_response_schema_bytes"
        ],
...
def evidence_manifest(validated: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "image_digest": runtime["container_image_digest"],
        "trust_bundle_bytes": runtime["trust_bundle_bytes"],
        "registered_response_schema_bytes": receipt["registered_response_schema_bytes"],
        "provider_response_schema_bytes": receipt["provider_response_schema_bytes"],
