...
## Complete synthesis and operational-model proposals — 2026-09-15

The [latest source-owned review](https://github.com/williamjblair/autonomous-science/blob/eb28ecd4de4fc62b532f7ef85410abc373f1cb78/reports/capability-fungibility-synthesis-2026-09-15/RESULTS.md) adds 78 original artifacts and 20 captured responses through Operational Compilation. Three intent simulations and the bounded six-client HTTP prototype replay locally; later workbooks retain reported output status and specific generator gaps. These source experiments may inform evidence scope. They do not define current product or protocol semantics.

For an externally produced operational-model or component claim, existing records can bind the declared interventions, observations, horizon, risk/coverage, preparation, implementation/configuration versions, dependency set and checker inputs. Changed scopes or newly distinguishing probes can require a source-owned correction/requalification. A redundant new probe need not invalidate a result; a wider observation family cannot silently inherit narrower qualification.

The model's declaration that an effect is final, authorized or safe is a checked assertion only within its stated assumptions. It cannot grant external effect permission or change Vela Standing. Finality in the imported intent simulator is explicitly supplied; zero observed harm with substantial abstention is not a universal guarantee. Local witness disagreement should remain attributed evidence unless the named Repository admits an authorized Decision; no global truth, sheaf object, operational compiler, BoundIntent object or institution runtime is added to core.

Characterization, learned abstractions, world-model fitting, experiment scheduling, effect execution and institutional mechanism selection remain source-owned. Same-author adapters and finite simulations do not provide the two maintained consumers or removed duplication needed for core extraction. The latest operational-discovery hypothesis remains open and does not change Protocol 1, authority, release state or the existing product loop.

The [latest operational code recovery](https://github.com/williamjblair/autonomous-science/blob/79e8866cefe108780556cc54ca9a0269712fe578/reports/capability-fungibility-synthesis-2026-09-15/CODE_RECOVERY.md) additionally reproduces all four operational workbook tables. Its exact latent-state probe responses and finite transition model are supplied inputs. The initial generator gap is superseded for that example; characterization/discovery and external authority remain outside the checked claim and Vela core.
