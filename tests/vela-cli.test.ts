...
function fakeRunner(options: FakeOptions = {}): {
  ...
  const runner: CommandRunner = async (command) => {
    ...
    if (argv[1] === "--version") {
      return result(argv, `vela ${options.version ?? "0.940.6"}\n`);
    }
    if (argv[1] === "status") {
...
function client(runner: CommandRunner, version = "0.940.6"): VelaClient {
