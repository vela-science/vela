...
| Producer identity-binding handle `vib_…` and full credential root | First 16 hex characters, or the complete lowercase SHA-256 root, over the Vela-canonical `vela.identity_binding.v0.1` object with `binding_id` and `signature` cleared. The self-signature separately proves possession of the embedded Ed25519 key. | `vib_` is an exact routing handle. The complete root is the credential identity used by the Proposed AcceptancePolicy v0.3 scoped allowlist. | Actor-registry membership, personhood, expertise, independence, or authority without an exact human-signed policy. |
