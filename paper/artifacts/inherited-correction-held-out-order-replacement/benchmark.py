...
ORDER_CONTRACT_AMENDMENT_PATH = ROOT / "order-contract-amendment.json"
RUN_PREFIX = "orderfix-run"
PARTICIPANT_PREFIX = "orderfix-sol"
CALIBRATION_RUN_ID = "neutral-orderfix-calibration-01"
CALIBRATION_PARTICIPANT_ID = "neutral-orderfix-sol-01"
CURRENT_STATUS = "held_pending_independent_order_review"
SOURCE_PATHS = (
    "DESIGN.md",
    "README.md",
    "TASK.md",
    ...
    "order-contract-amendment.json",
    "offline-preflight/provider-schema/provider-events.jsonl",
    "offline-preflight/provider-schema/receipt.json",
...
def provider_schema() -> dict[str, Any]:
    """Exact provider-only derivative; the registered schema remains unchanged."""
    value = load_json(SCHEMA_PATH)
    bindings = value.get("properties", {}).get("evidence_bindings", {})
    if bindings.get("uniqueItems") is not True:
        raise BenchmarkError("registered_unique_items_missing")
    del bindings["uniqueItems"]
    return value
...
def calibration_fixture() -> dict[str, Any]:
    paths = [f"evidence/neutral-{letter}.json" for letter in "abcd"]
    bindings = [
        {"path": path, "sha256": byte_digest(f"neutral-{index}\n".encode())}
        for index, path in enumerate(paths, 1)
    ]
    response = {
        "schema": "inherited-correction-held-out-response.v1",
        "family_id": "neutral-schema-compatibility",
        "predecessor_claim_id": "neutral-old",
        "successor_claim_id": "neutral-new",
        "consequences": [
            {
                "claim_id": f"neutral-{letter}",
    ...
    prompt = json_bytes(
        {
            "schema": "inherited-correction-neutral-schema-calibration.v1",
            "instruction": "Return exactly the supplied response object as one JSON object. This is a non-scientific schema-surface calibration with no study packet.",
            "response": response,
        }
    )
    packet_root = canonical_root(
        [
            {"path": "neutral-prompt.json", "sha256": byte_digest(prompt)},
            {
                "path": "response-schema.json",
                "sha256": byte_digest(SCHEMA_PATH.read_bytes()),
            },
        ]
    )
    return {"prompt": prompt, "response": response, "packet_root": packet_root}
        ...
        assignments.append(
            {
                **cell,
                ...
                "participant_instance_id": f"{PARTICIPANT_PREFIX}-{index:02d}",
            }
        )
    return {
...
def runtime_configuration(
    ...
    return {
        ...
        "provider_response_schema_bytes": byte_digest(json_bytes(provider_schema())),
...
def generated_outputs() -> dict[str, bytes]:
    ...
    for family_id, family in sorted(families.items()):
        ...
        for condition in CONDITIONS:
            ...
            outputs[
                f"conditions/{family_id}/{condition}/input/provider-response-schema.json"
            ] = json_bytes(provider_schema())
        common_sources = [
            file_entry(path, raw) for path, raw in sorted(source_files(family).items())
        ]
    ...
    calibration_plan = {
        "schema": "vela.inherited-correction-neutral-schema-calibration-plan.v1",
        "run_id": CALIBRATION_RUN_ID,
        "participant_instance_id": CALIBRATION_PARTICIPANT_ID,
        "condition": "neutral-calibration",
        "prompt_root": byte_digest(calibration["prompt"]),
        "packet_root": calibration["packet_root"],
        "registered_response_schema_bytes": byte_digest(SCHEMA_PATH.read_bytes()),
        "provider_response_schema_bytes": byte_digest(json_bytes(provider_schema())),
        "expected_response_root": canonical_root(calibration["response"]),
    ...
    order_contract_amendment = load_json(ORDER_CONTRACT_AMENDMENT_PATH)
    registration = {
        ...
        "bindings": {
            ...
            "runtime_test_bytes": byte_digest(
                (ROOT / "test_provider_schema_runtime.py").read_bytes()
            ),
            "response_schema_bytes": byte_digest(SCHEMA_PATH.read_bytes()),
            "provider_response_schema_bytes": byte_digest(
                json_bytes(provider_schema())
            ),
            "provider_schema_derivation": "delete exactly /properties/evidence_bindings/uniqueItems for provider output only; validate the final response against the unchanged registered schema",
            "task_bytes": byte_digest(TASK_PATH.read_bytes()),
            ...
            "order_contract_amendment": order_contract_amendment,
    ...
    calibration_config = runtime_configuration(
        registration_root, calibration_plan["prompt_root"], runtime
    )
    calibration_config["expected_response_root"] = calibration_plan[
        "expected_response_root"
    ]
    calibration_assignment = {
        "schema": "vela.inherited-correction-neutral-schema-calibration-assignment.v1",
        "registration_root": registration_root,
        "image_digest": runtime["container_image_digest"],
        "assignments": [
            {
                "run_id": calibration_plan["run_id"],
                "participant_instance_id": calibration_plan["participant_instance_id"],
                "condition": calibration_plan["condition"],
                "packet_root": calibration_plan["packet_root"],
            }
        ],
    }
    calibration_config_root = canonical_root(calibration_config)
    calibration_assignment_root = canonical_root(calibration_assignment)
    outputs["calibration/input/prompt.txt"] = calibration["prompt"]
    outputs["calibration/input/response-schema.json"] = SCHEMA_PATH.read_bytes()
    outputs["calibration/input/provider-response-schema.json"] = json_bytes(
        provider_schema()
    )
    outputs["calibration/input/valid-response.json"] = json_bytes(
        calibration["response"]
    )
    outputs["calibration/input/participant-configuration.json"] = json_bytes(
        calibration_config
    )
    outputs["calibration/input/assignment.json"] = json_bytes(calibration_assignment)
    calibration_hold = {
        "schema": "vela.inherited-correction-hold.v1",
        "status": "hold",
        "reason": "Independent prelaunch PASS required before the single neutral calibration permit may be released.",
        "updated_at": load_json(SEED_PATH)["generated_at"],
    }
    outputs["calibration/permit-template/hold-state.json"] = json_bytes(
        calibration_hold
    )
    calibration_permit = {
        ...
        "registration_root": registration_root,
        "image_digest": runtime["container_image_digest"],
        "assignment_root": calibration_assignment_root,
        "participant_configuration_root": calibration_config_root,
        "run_id": calibration_plan["run_id"],
        "participant_instance_id": calibration_plan["participant_instance_id"],
        "condition": calibration_plan["condition"],
        "packet_root": calibration_plan["packet_root"],
        "prompt_root": calibration_plan["prompt_root"],
        "trust_bundle_bytes": runtime["trust_bundle_bytes"],
        "attempt": 1,
    }
    outputs[
        f"calibration/permit-template/{CALIBRATION_RUN_ID}.permit.json"
    ] = json_bytes(calibration_permit)
    ...
    hold = {
        "schema": "vela.inherited-correction-hold.v1",
        "status": "hold",
        "reason": "Exact schema-compatibility replacement review PASS required; no permit release or provider call authorized before PASS.",
        "updated_at": load_json(SEED_PATH)["generated_at"],
    }
    ...
    freeze = {
        ...
        "provider_response_schema_bytes": byte_digest(json_bytes(provider_schema())),
        "neutral_calibration": {
            "plan_root": canonical_root(calibration_plan),
            "configuration_root": calibration_config_root,
            "assignment_root": calibration_assignment_root,
            "permit_root": canonical_root(calibration_permit),
            "status": "held",
            "provider_calls": 0,
            "denominator_credit": False,
        },
        "permit_set_root": canonical_root(
            [
                ...
                if path.startswith(f"permit-template/{RUN_PREFIX}-")
            ]
        ),
...
def write_outputs() -> None:
    ...
    for relative in ("conditions", "permit-template", "calibration/permit-template"):
...
def verify() -> None:
    ...
    if (
        amendment["status"] != "authorized_held_pending_schema_compatibility_review"
...
def validate_response(
    ...
    expected_ids = {item["claim_id"] for item in family["consequences"]}
    ...
    for item in consequences:
        ...
        if not isinstance(item["claim_id"], str) or not item["claim_id"]:
            raise BenchmarkError("response_claim_id_invalid")
    ...
    observed_ids = [item["claim_id"] for item in consequences]
    if len(set(observed_ids)) != len(observed_ids):
        raise BenchmarkError("response_claim_id_duplicate")
    if set(observed_ids) != expected_ids:
        raise BenchmarkError("response_claim_id_set_invalid")
    known = {
    ...
    consequence_by_id = {item["claim_id"]: dict(item) for item in consequences}
    binding_by_pair = {
        (binding["path"], binding["sha256"]): dict(binding) for binding in bindings
    }
    evidence_pair_by_id = {
        item["claim_id"]: (
            item["evidence_path"],
            known[item["evidence_path"]],
        )
        for item in family["consequences"]
    }
    normalized = dict(response)
    normalized["consequences"] = [
        consequence_by_id[claim_id] for claim_id in sorted(expected_ids)
    ]
    normalized["evidence_bindings"] = [
        binding_by_pair[evidence_pair_by_id[claim_id]]
...
def score_response(
    ...
    response = validate_response(response, family, packet_manifest)
