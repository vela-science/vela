...
#[derive(Serialize)]
struct McpServers<'a> {
    #[serde(rename = "vela-local")]
    vela_local: McpServer<'a>,
}
...
fn mcp_json_adapter(root: &Path) -> String {
    // Pick a serve invocation from durable worktree structure. A tracked
    // `frontier.yaml` identifies a single frontier even when private `.vela/`
    // state is absent from a clean clone; consulting `.vela/` alone made
    // generated adapters differ between local and CI checkouts. Multi-frontier
    // roots serve their conventional catalog (`frontiers/`, else `examples/`).
    // Draft profile: the read surface plus the non-finalizing `work` actions.
    // `decide` is excluded by construction, so
    // this delegates nothing — matching what the skill teaches. Keep the
    // profile and the skill text in step.
    let args: Vec<String> = if root.join("frontier.yaml").is_file() || root.join(".vela").is_dir() {
        ["serve", ".", "--profile", "draft"]
            .iter()
            .map(|s| s.to_string())
            .collect()
    ...
    // Serialize typed structs rather than `serde_json::Value`. A Value map can
    // preserve insertion order or sort keys depending on Cargo feature
    // unification, which previously made release and debug binaries disagree
    // about the generated bytes.
    let value = McpAdapter {
        generated_by: "vela agents sync (from VELA.md) — edit VELA.md, not this file",
        mcp_servers: McpServers {
            vela_local: McpServer {
                args: &args,
                command: "vela",
            },
        },
...
mod tests {
    ...
    #[test]
    fn mcp_adapter_uses_tracked_frontier_manifest() {
        let tmp = tempfile::TempDir::new().unwrap();
        std::fs::write(tmp.path().join("frontier.yaml"), "schema: test\n").unwrap();

        let generated = mcp_json_adapter(tmp.path());
        let value: serde_json::Value = serde_json::from_str(&generated).unwrap();
        assert_eq!(
            value["mcpServers"]["vela-local"]["args"],
            serde_json::json!(["serve", ".", "--profile", "draft"])
        );
        assert!(
            generated.find("\"args\"").unwrap() < generated.find("\"command\"").unwrap(),
            "typed adapter serialization must not depend on serde_json map ordering"
        );
    }
}
