...
| `AuthorityRecordV1.content.authentication` (`AuthorityRecordContentV1.authentication`, the `AuthenticationClaimV1` alias of `AuthenticationObservationV1`) | The authentication provider owns the native ceremony, credentials, and mutable provider session; the Vela Repository owns the exact bearer-free observation admitted into its authority history | Recompute authorization against the same authenticated principal and exact observation used by the admitted transaction | **Copy** the complete canonical bearer-free observation: `schema`, `principal_id`, `principal_class`, `issuer`, `subject`, `method`, `assurance`, `session_root`, `authenticated_at`, `observed_at`, `expires_at`, `user_presence`, `user_verification`, `recovery_recent`, and `revocation_ref`. `session_root` and `revocation_ref` are opaque replay inputs/references, not custody of provider session or revocation systems | Credentials, cookies, bearer tokens, provider sessions, private keys, mutable IdP account state, or a provider session/revocation system |
