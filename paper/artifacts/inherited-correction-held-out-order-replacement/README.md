# Order-invariant held-out inherited-correction benchmark

This non-authoritative paper artifact is a fresh prospective successor to the
stopped order-sensitive replacement registration and, before it, the stopped
36-cell launch whose sole run was retained as a provider-schema non-result.
Those stopped records and the sealed earlier 16-session study with
`positive_gate=not_supported` remain unchanged. No participant or calibration
call has been made under this fresh successor registration.
...
The first participant call under the independently qualified predecessor
registration exposed an order-contract defect. The registered schema accepts
the four closed consequence objects in any order and the task prescribes no
array ordering, but custody required alphabetically sorted claim IDs. That
exact terminal response and consumed permit are frozen in the separate stopped
execution record with no retry or replacement credit; the remaining 35
predecessor permits may never be released.

This prospective successor treats consequence order as semantically
irrelevant. It retains response bytes exactly, requires the unique closed set
of four expected claim IDs with exact fields and evidence bindings, and sorts
only a derived validation/scoring representation by claim ID. Missing,
duplicate, and unknown IDs fail closed. Because the predecessor exposed one
cell, this successor has a fresh registration, random assignment seed, run and
participant identities, and 36 new held permits. A fresh neutral calibration
is also held: the prior calibration binds the predecessor registration and
prior benchmark/custody bytes. No new permit may be released until this exact
successor receives independent review PASS, after which the fresh calibration
must independently pass before participant launch.
