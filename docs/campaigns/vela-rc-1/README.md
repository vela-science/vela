# VELA-RC-1 campaign index

VELA-RC-1 qualifies the exact VELA-COMPOSE-1 integrated candidate for a
possible external release. It is a release and adoption-readiness campaign,
not a foundational-research campaign.

- [Campaign charter](CAMPAIGN.md)
- [Frozen baseline](BASELINE.md)
- [Append-only state](STATE.md)
- [Append-only decisions](DECISIONS.md)
- [Qualification matrix](QUALIFICATION.md)
- [R2 clean-install and replay qualification](R2_CLEAN_INSTALL_QUALIFICATION.md)
- [R2 clean-install and replay requalification](R2_REQUALIFICATION.md)
- [R3 CLI and first-user documentation qualification](R3_FIRST_USER_QUALIFICATION.md)
- [R4 external-use fixtures qualification](R4_EXTERNAL_FIXTURES.md)
- [R5 product and Observatory semantic-legibility audit](R5_PRODUCT_LEGIBILITY.md)
- [R5 independent product requalification](R5_REQUALIFICATION.md)
- [R6 packaging and release-integrity qualification](R6_RELEASE_INTEGRITY.md)
- [R6 bounded packaging repair](R6_PACKAGING_REPAIR.md)
- [Independent R6 packaging requalification](R6_REQUALIFICATION.md)
- [Second independent R6 requalification](R6_SECOND_REQUALIFICATION.md)
- [Failure ledger](FAILURES.md)
- [R1 protocol and semantic audit](R1_SEMANTIC_AUDIT.md)
- [R1 independent requalification](R1_REQUALIFICATION.md)
- [Release checklist](RELEASE_CHECKLIST.md)
- [Blind external-user test](EXTERNAL_TEST.md)
- [Final release decision](FINAL.md)

No tag, publication, push, version bump, outreach, or release is authorized by
these records.
