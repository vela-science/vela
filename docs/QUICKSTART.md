...
historical-correction projection patch is qualified. The following command
becomes valid only after that tag is signed and published; the installer then
verifies the exact platform manifest with the out-of-band distribution
identity.
