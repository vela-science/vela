...
| Authority trust-anchor root | SHA-256 of the minimal closed local `vela.authority-trust-anchor.v1` record containing the Frontier ID and independently obtained full sequence-1 authority-record root. | Full local-anchor root plus an exact match between its `first_authority_record_root` and the first record of a completely verified authority chain. | Repository-boundary trust, a secret key, repository or scientific authority, later-chain freshness, or scientific state. |
