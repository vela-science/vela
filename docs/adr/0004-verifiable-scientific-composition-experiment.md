...
| Git/DSSE/in-toto same-information baseline | **PROVED — INTERNAL** | one fixture-signed non-authority `science.lock`, unsigned DSSE wrapper, in-toto Statement, exact offline bundle, and 13 hostile wrapper vectors carry the same canonical fact manifest; the simpler profile remains eligible to win |
| First-party resolver, CI, and context projections | **PROVED — LOCAL** | 54 hostile vectors, seven three-consumer CLI cases, independent Reader C status/root parity, offline delivery classification, and projection deletion all pass; combined standing changes are rejected rather than resolved by hidden precedence |
